...
for skill in pge pge-strict pge-quality pge-ultra pge-idontcaretokenanymore pge-god pge-eval-backend; do
...
for variant in evaluator-standard evaluator-strict evaluator-quality evaluator-godmode evaluator-codex evaluator-gemini pge-orchestrator; do
...
echo ""
echo "=== Installation complete (v2.6.0) ==="
echo ""
...
echo '  /pge-eval-backend       — select evaluator backend (claude/codex/gemini)'
echo ""
...
echo '  @evaluator-codex   — Codex CLI evaluator (static code review, no Playwright)'
echo '  @evaluator-gemini  — Gemini CLI evaluator (static code review, no Playwright)'
echo '  @pge-orchestrator'
echo ""
echo "Eval backend:"
echo '  /pge-eval-backend codex    — use Codex CLI for evaluation'
echo '  /pge-eval-backend gemini   — use Gemini CLI for evaluation'
echo '  /pge-eval-backend claude   — restore full Playwright evaluation (default)'
